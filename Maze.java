 ...
 *  1) From current position, go either up down left or right
    2) If the current position is either not a path or outside the board, end the current call and go another direction instead
    3) If the current position is the exit, then print the final board and exit the whole animation
 ***/
...
class MazeSolver 
{
  ...
  public void solve( int x, int y ) {
    ...
    else if (x > w || y > h || x < 0 || y < 0) { //makes sure it doesn't go OOB
      return;
    }

    else if ( maze[x][y] == EXIT) { //reaches exit
	    solved = true;
	    System.out.println(this);
      return;
    }
    else if ( maze[x][y] != PATH  )   { //if it steps out of the path
	    return;
    }

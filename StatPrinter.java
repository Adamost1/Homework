...
public class StatPrinter {
    ...
    {
	    ArrayList<Integer> arrayModes = new ArrayList<Integer>();

      for(Integer c: _frequency){
        if(isLocalMode(c)){
          arrayModes.add(c);
        }
      }
    ...
    public void printHistogram( int longestBar ) // O(n^2)
    {
      for(int i = 0; i < _frequency.size(); i ++){
        ...
        for(int counter = 0; counter < (int) ( (double) _frequency.get(i) * (longestBar/max(_frequency)) ); counter ++ ){
          System.out.print("*");
        } 
        System.out.println();
      }
